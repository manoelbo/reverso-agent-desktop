...
import { timelineActionCommands, type TimelineRow } from './timeline-mocks'

type TimelineTableShellProps = {
  rows: TimelineRow[]
}
...
function FolderCellButton({ value, onActivate }: { value: string; onActivate: (value: string) => void }): React.JSX.Element {
  return (
    <button
      type='button'
...
export function TimelineTableShell({ rows }: TimelineTableShellProps): React.JSX.Element {
  ...
  const activeFiltersCount = yearFilters.length + monthFilters.length

  const visibleRows = useMemo(() => {
    return rows.filter((row) => {
      const searchMatch = row.title.toLowerCase().includes(search.toLowerCase())
      const yearMatch = !yearFilters.length || yearFilters.includes(row.year)
      const monthMatch = !monthFilters.length || monthFilters.includes(row.month)
      return searchMatch && yearMatch && monthMatch
    })
  }, [rows, search, yearFilters, monthFilters])
              ...
              <div className='space-y-2'>
                ...
                <div className='max-h-80 space-y-1 overflow-y-auto pr-1'>
                  ...
                  {filteredYearOptions.map((value) => (
                    <DropdownMenuCheckboxItem
                      key={`year-${value}`}
                      checked={yearFilters.includes(value)}
                  ...
                  {filteredMonthOptions.map((value) => (
                    <DropdownMenuCheckboxItem
                      key={`month-${value}`}
                      checked={monthFilters.includes(value)}
              ...
              {!visibleRows.length ? (
                <tr>
                  <td colSpan={7} className='px-3 py-6 text-center text-sm text-muted-foreground'>
                    No timeline entries matched your current filters.
                  </td>
                </tr>
              ) : null}
            </tbody>
...
export function TimelinePageHeader({ title, subtitle }: TimelineHeaderProps): React.JSX.Element {
  return (
...
export function TimelineSummaryBadge({ rows }: { rows: TimelineRow[] }): React.JSX.Element {
  const years = useMemo(() => new Set(rows.map((row) => row.year)).size, [rows])

  return (
    <Badge variant='outline' className='h-8 rounded-md px-2 text-xs'>
      {rows.length} entries · {years} years
    </Badge>
  )
}
