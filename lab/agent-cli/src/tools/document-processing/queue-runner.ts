...
function resolveFilesystemRootFromSourceDir(sourceDirAbs: string): string {
  const base = path.basename(sourceDirAbs).toLowerCase()
  if (base === 'source' || base === 'sources') {
    return path.dirname(sourceDirAbs)
  }
  return sourceDirAbs
}

function toStepId(label: string): string {
  return `doc-process-${label.replace(/[^a-zA-Z0-9_-]+/g, '-').toLowerCase()}`
}

function feedbackStep(
  feedback: UiFeedbackController | null,
  label: string,
  status: 'in_progress' | 'completed' | 'blocked',
  details?: string
): void {
  if (!feedback) return
  const stepId = toStepId(label)
  if (status === 'in_progress') {
    feedback.stepStart(stepId, label, details)
    return
  }
  if (status === 'completed') {
    feedback.stepComplete(stepId, details)
    return
  }
  feedback.stepError(stepId, details)
}

function feedbackInfo(feedback: UiFeedbackController | null, message: string): void {
  if (!feedback) return
  feedback.systemInfo(message)
}

function feedbackWarn(feedback: UiFeedbackController | null, message: string): void {
  if (!feedback) return
  feedback.systemWarn(message)
}

function isNonRetryableRuntimeError(message: string): boolean {
  return message.includes(PDF_LOCAL_RUNTIME_ERROR_CODE)
}

function isRetriableSourceStatus(status: string): boolean {
  return (
    status === 'not_processed' ||
    status === 'failed' ||
    status === 'replica_paused' ||
    status === 'preview_metadata_paused'
  )
}

function feedbackFileChange(
  feedback: UiFeedbackController | null,
  change: { path: string; changeType: 'new' | 'edited' | 'deleted'; preview?: string }
): void {
  if (!feedback) return
  if (change.changeType === 'new') {
    feedback.fileCreated(change.path, 0, change.preview)
    return
  }
  if (change.changeType === 'edited') {
    feedback.fileEdited(change.path, 0, 0)
    return
  }
  feedback.fileDeleted(change.path)
}
...
export async function runQueue(options: QueueRunnerOptions): Promise<void> {
  ...
  const filesystemRoot = resolveFilesystemRootFromSourceDir(sourceDirAbs)
    ...
    options.mode === 'queue'
      ? checkpoint.files.filter(
          (f) =>
            f.fileType === 'pdf' &&
            f.queuedAt != null &&
            isRetriableSourceStatus(f.status)
        )
      : options.mode === 'all'
        ? checkpoint.files.filter(
            (f) =>
  ...
  for (const f of pdfs.filter((x) => x.status === 'failed' || x.status === 'replica_paused' || x.status === 'preview_metadata_paused')) {
  ...
  const toProcess = pdfs.filter((f) => isRetriableSourceStatus(f.status))
  ...
  const eventsDir = path.join(filesystemRoot, 'events')
  ...
  // Paths operacionais (dossier/investigation/events) para ambos os modos.
  let labPaths = null
  try {
  ...
  for (let i = 0; i < toProcess.length; i += 1) {
    ...
    feedbackStep(feedback, `[${current}/${total}] ${entry.originalFileName}`, 'in_progress', processingMode)
    ...
    while (!completed && attempt < DOC_MAX_ATTEMPTS) {
      ...
      try {
        if (processingMode === 'standard') {
          const dossierDir = labPaths?.dossierDir ?? path.join(filesystemRoot, 'dossier')
          ...
          const result = await runStandardProcess({
            ...
            onStepStart: (step) => {
              ...
              feedbackStep(feedback, `${entry.originalFileName} :: ${step}`, 'in_progress')
            },
            onStepDone: (step) => {
              console.log(`  [${step}] concluido.`)
              feedbackStep(feedback, `${entry.originalFileName} :: ${step}`, 'completed')
            },
            onStepError: (step, err) => {
              console.error(`  [${step}] erro: ${err.message}`)
              feedbackStep(feedback, `${entry.originalFileName} :: ${step}`, 'blocked', err.message)
            },
            onInfo: (message) => feedbackInfo(feedback, message),
            onArtifact: (artifact) => {
              feedbackFileChange(feedback, {
          ...
          await processSingleDocument({
            ...
            onReplicaStart: () => {
              ...
              feedbackStep(feedback, `${entry.originalFileName} :: replica`, 'in_progress')
            },
            onReplicaDone: () => {
              console.log(`  Réplica: concluída.`)
              feedbackStep(feedback, `${entry.originalFileName} :: replica`, 'completed')
            },
          ...
          const dossierDir = labPaths?.dossierDir ?? path.join(filesystemRoot, 'dossier')
          const replicaMarkdown = await readFile(replicaPath, 'utf8')
          console.log(`  Dossier: materializando entidades...`)
          feedbackStep(
            feedback,
            `${entry.originalFileName} :: dossier-from-replica`,
            'in_progress'
          )
          const dossierResult = await runStandardDossierFromReplica({
            ...
            artifactLanguage: options.artifactLanguage ?? 'source',
            onStepStart: (step) =>
              feedbackStep(feedback, `${entry.originalFileName} :: dossier-${step}`, 'in_progress'),
            onStepDone: (step) =>
              feedbackStep(feedback, `${entry.originalFileName} :: dossier-${step}`, 'completed'),
            onStepError: (step, err) =>
              feedbackStep(
                feedback,
                `${entry.originalFileName} :: dossier-${step}`,
                'blocked',
                err.message
              ),
            onInfo: (message) => feedbackInfo(feedback, message),
            onArtifact: (artifact) => {
              feedbackFileChange(feedback, {
                path: relPath(artifact.path),
                changeType: artifact.changeType,
                preview: `${entry.originalFileName} :: dossier-${artifact.step}`
              })
            }
          })
          feedbackStep(
            ...
            `${entry.originalFileName} :: dossier-from-replica`,
            'completed',
            JSON.stringify(dossierResult.usage)
          )
          console.log(`  Dossier: concluído.`)
          ...
          await markSourceStatus(sourceDirAbs, entry.docId, 'done', {
            ...
            processingSummary: {
              totalPagesInPdf: lastReport.totalPagesInPdf,
              totalChunks: lastReport.totalChunks,
              usage: mergeUsage([lastReport.usage, dossierResult.usage]),
              model: options.model,
              previewModel: options.previewModel ?? options.model
            }
          })
        ...
        feedbackStep(feedback, entry.originalFileName, 'completed')
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err)
        console.error(`  Erro: ${message}`)
        const nonRetryable = isNonRetryableRuntimeError(message)
        const hasRetry = !nonRetryable && attempt < DOC_MAX_ATTEMPTS
        ...
        if (!hasRetry) {
          ...
          if (nonRetryable) {
            const mitigation =
              `${entry.originalFileName}: local parser unavailable (DOMMatrix/canvas). ` +
              `Use "reverso process --mode deep" or "reverso process-all --mode deep".`
            feedbackWarn(feedback, mitigation)
            console.warn(`  Mitigacao: ${mitigation}`)
          }
          feedbackStep(feedback, entry.originalFileName, 'blocked', message)
          break
        }
        feedbackWarn(
          feedback,
