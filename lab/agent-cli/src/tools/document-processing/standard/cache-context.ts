...
/**
 * Cria um CacheContext a partir de uma replica já pronta (modo deep),
 * permitindo reaproveitar as etapas de extração de entidades do Standard.
 */
export function buildCacheContextFromReplica(input: {
  replicaMarkdown: string
  model: string
  sourceFileName: string
}): CacheContext {
  const promptFromReplica = [
    'Documento já processado previamente para texto Markdown.',
    `Fonte: ${input.sourceFileName}`,
    'Use este conteúdo como base para extrações estruturadas.'
  ].join('\n')

  return {
    pdfUserMessage: {
      role: 'user',
      content: promptFromReplica
    },
    firstAssistantMessage: {
      role: 'assistant',
      content: input.replicaMarkdown,
      annotations: []
    },
    annotations: [],
    model: input.model,
