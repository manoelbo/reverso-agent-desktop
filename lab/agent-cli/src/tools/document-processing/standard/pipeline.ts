...
import { buildCacheContextFromReplica } from './cache-context.js'
import {
...
export interface StandardDossierFromReplicaParams {
...
/**
 * Materializa entidades do dossier (persons/groups/places/events) a partir de replica.md.
 * Reutiliza os mesmos steps do Standard Process sem depender do parse nativo de PDF.
 */
export async function runStandardDossierFromReplica(
  params: StandardDossierFromReplicaParams
): Promise<StandardProcessResult> {
  await mkdir(params.artifactDir, { recursive: true })
  const model = params.model ?? DEFAULT_MODEL
  const artifactLanguageInstruction = buildArtifactLanguageInstruction(
    params.artifactLanguage ?? 'source'
  ...
  const client = new OpenRouterClient(params.apiKey)
  const ctx: CacheContext = buildCacheContextFromReplica({
    replicaMarkdown: params.replicaMarkdown,
    model,
    sourceFileName: params.sourceFileName
  })

  const usages: OpenRouterUsage[] = []
  const artifacts: StandardProcessCheckpoint['artifacts'] = {
    notes: [],
    personsCreated: [],
    personsUpdated: [],
    groupsCreated: [],
  ...
  const dossierSteps: Array<{ name: StepName; run: () => Promise<void> }> = [
    {
      name: 'persons',
      run: async () => {
        const result = await runStepPersons({
        ...
        usages.push(result.usage)
        artifacts.personsCreated = [...artifacts.personsCreated, ...result.created]
        artifacts.personsUpdated = [...artifacts.personsUpdated, ...result.updated]
        for (const filePath of result.created) {
          params.onArtifact?.({ step: 'persons', path: filePath, changeType: 'new' })
        }
        for (const filePath of result.updated) {
    ...
    },
    {
      ...
      run: async () => {
        ...
        usages.push(result.usage)
        artifacts.groupsCreated = [...artifacts.groupsCreated, ...result.created]
        artifacts.groupsUpdated = [...artifacts.groupsUpdated, ...result.updated]
        for (const filePath of result.created) {
          params.onArtifact?.({ step: 'groups', path: filePath, changeType: 'new' })
        }
        for (const filePath of result.updated) {
    ...
    },
    {
      ...
      run: async () => {
        ...
        usages.push(result.usage)
        artifacts.placesCreated = [...artifacts.placesCreated, ...result.created]
        artifacts.placesUpdated = [...artifacts.placesUpdated, ...result.updated]
        for (const filePath of result.created) {
          params.onArtifact?.({ step: 'places', path: filePath, changeType: 'new' })
        }
        for (const filePath of result.updated) {
    ...
    },
    {
      ...
      run: async () => {
        ...
        usages.push(result.usage)
        artifacts.eventsPaths = [...new Set([...artifacts.eventsPaths, ...result.eventsPaths])]
        for (const filePath of result.created) {
          params.onArtifact?.({ step: 'events', path: filePath, changeType: 'new' })
        }
        for (const filePath of result.eventsPaths) {
  ...
  for (const step of dossierSteps) {
    params.onStepStart?.(step.name)
    try {
      await step.run()
      params.onStepDone?.(step.name)
  ...
  params.onInfo?.('Dossier materializado a partir da replica (modo deep).')
  return { usage: mergeUsage(usages), artifacts }
}
