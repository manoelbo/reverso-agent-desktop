...
import { runInquiryAll } from '../runner/run-inquiry-all.js'
...
export const inquiryAllCommand: CommandModule<object, any> = {
  command: 'inquiry-all',
  describe: 'Run inquiry on all uninvestigated leads',
  ...
  handler: async (argv) => {
    ...
    await runInquiryAll({
      ...
      ...(typeof argv['p2-evidence-mode'] === 'string'
        ? {
            p2EvidenceVerificationMode: argv['p2-evidence-mode'] as EvidenceVerificationMode
          }
        : {}),
      ...
      ...(typeof sensitivePolicyMode === 'string'
        ? {
            p2SensitiveDataPolicyMode: sensitivePolicyMode as SensitiveDataPolicyMode
          }
        : {})
    })
  }
