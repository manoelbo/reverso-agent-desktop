...
export function AllegationViewPanel({
  investigationIndex,
  investigationIndexLoading,
  investigationIndexError,
  investigationIndexStale,
  onOpenAllegationDocument,
}: AllegationViewPanelProps): JSX.Element {
  const [search, setSearch] = useState("")
  const rows = investigationIndex?.allegations ?? []
  ...
  const linkedFindings = rows.reduce((acc, row) => acc + row.findingIds.length, 0)
  ...
  return (
    <div className="mx-auto flex min-h-full w-full max-w-7xl flex-col gap-4 px-8 py-8">
      <header className="space-y-1 rounded-2xl border border-border/70 bg-card/70 px-6 py-5 shadow-sm">
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Allegations</h1>
        <p className="text-sm text-muted-foreground">Claims under review, linked to leads and supporting findings.</p>
      </header>
      ...
      <section className="grid grid-cols-2 gap-3 rounded-2xl border border-border/70 bg-card/60 p-4 shadow-sm md:grid-cols-4">
        ...
        <div className="rounded-lg border border-red-500/20 bg-red-500/5 px-3 py-2">
          <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Linked Findings</p>
          <p className="text-xl font-semibold text-red-700 dark:text-red-300">{linkedFindings}</p>
        </div>
        <div className="rounded-lg border border-border/60 bg-background/70 px-3 py-2">
        ...
        <div className="rounded-lg border border-border/60 bg-background/70 px-3 py-2">
          <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Updated Docs</p>
      ...
      <section className="space-y-3 rounded-2xl border border-border/70 bg-card/70 p-4 shadow-sm">
        <div className="relative min-w-[18rem] max-w-lg">
          ...
          <Input
            ...
            className="pl-8"
            placeholder="Search allegations by title, id, lead or filename"
          />
        </div>
          ...
          <table className="w-full min-w-[980px] border-collapse text-sm">
            <thead className="sticky top-0 z-10 bg-muted/60 text-xs uppercase tracking-wide text-muted-foreground backdrop-blur">
              <tr>
                ...
                <th className="w-40 px-3 py-2 text-left">Linked Findings</th>
            ...
            <tbody>
              {visibleRows.map((row) => (
                <tr key={row.relativePath} className="border-t border-border/60 transition-colors hover:bg-muted/25">
                  ...
                  <td className="px-3 py-2 align-middle text-muted-foreground">{row.findingIds.length}</td>
                  ...
                  <td className="px-4 py-2 align-middle text-right">
                    <button
                      ...
                      onClick={() => onOpenAllegationDocument(row.relativePath)}
                    >
                      View
                    </button>
                  </td>
              ...
              {!visibleRows.length ? (
                <tr>
                  <td colSpan={6} className="px-3 py-6 text-center text-sm text-muted-foreground">
                    No allegations matched your current search.
                  </td>
                </tr>
              ) : null}
            </tbody>
        ...
        {investigationIndexLoading ? <p className="text-xs text-muted-foreground">Loading allegations index...</p> : null}
        {investigationIndexError ? <p className="text-xs text-destructive">Index error: {investigationIndexError}</p> : null}
        {investigationIndexStale ? <p className="text-xs text-muted-foreground">Refreshing allegations data...</p> : null}
      </section>
    </div>
  )
}
